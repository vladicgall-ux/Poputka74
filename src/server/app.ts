...
export function createApp() {
  ...
  app.use(
    helmet({
      ...
      // HSTS: приложение и так работает только по HTTPS (кука сессии
      // помечена Secure), заголовок закрепляет это на стороне браузера.
      // Без preload и includeSubDomains: preload — это необратимая на
      // месяцы запись в списки браузеров, а поддомены домена от хостинга
      // нам не принадлежат.
      hsts: { maxAge: 15552000, includeSubDomains: false, preload: false },
    })
  );

  // Permissions-Policy helmet не выставляет вовсе. Приложению не нужны ни
  // камера, ни микрофон, ни геолокация, ни платёжный API — отключаем их
  // явно, чтобы этих возможностей не было и у любого стороннего кода,
  // который каким-то образом окажется на странице.
