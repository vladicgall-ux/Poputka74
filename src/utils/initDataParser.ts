...
export interface ParsedInitData {
  ok: true;
  /** Строка, по которой считается HMAC (все пары кроме hash, по ключу). */
  dataCheckString: string;
  hash: string;
  authDate: number;
  /** Сырой JSON поля user — разбирает уже вызывающий код. */
  userRaw: string;
}

export interface RejectedInitData {
  ok: false;
  /** Короткая причина для лога. Не содержит самих данных: в initData
   *  лежат персональные данные пользователя и подпись. */
  reason: string;
}
...
export function parseInitData(
...
): ParsedInitData | RejectedInitData {
  ...
  if (!authDateRaw) return { ok: false, reason: 'нет auth_date' };
  ...
  if (authDate > nowSec + CLOCK_SKEW_SEC) {
  ...
  if (!userRaw) return { ok: false, reason: 'нет user' };
