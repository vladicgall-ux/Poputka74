...
import { createRating, createPassengerRating, RatingError } from '../../services/ratingService';
...
ratingsRouter.post('/passenger', writeLimiter(20, 10 * 60_000), (req, res) => {
  const { user } = req as AuthedRequest;
  const rideId = Number(req.body?.rideId);
  ...
  try {
    const record = createPassengerRating({ rideId, driverId: user.telegram_id, passengerId, rating, comment });
    res.status(201).json({ rating: record });
  } catch (err) {
    if (err instanceof RatingError) {
      res.status(400).json({ error: err.message });
